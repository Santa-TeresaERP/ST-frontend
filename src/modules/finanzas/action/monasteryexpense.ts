...
export const fetchMonasteryExpenses = async (): Promise<MonasteryExpense[]> => {
  const response = await api.get<MonasteryExpense[]>(MONASTERY_EXPENSES_ENDPOINT);
...
export const fetchMonasteryExpenseById = async (id: string): Promise<MonasteryExpense> => {
  const response = await api.get<MonasteryExpense>(`${MONASTERY_EXPENSES_ENDPOINT}/${id}`);
  return response.data;
};
...
export const createMonasteryExpense = async (
  ...
  const response = await api.post<MonasteryExpense>(MONASTERY_EXPENSES_ENDPOINT, payload);
  return response.data;
};
...
export const updateMonasteryExpense = async (
  ...
  const response = await api.patch<MonasteryExpense>(
