...
interface ModalDeleteIngresoGastoProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: () => void;
  type: 'ingreso' | 'gasto';
...
const ModalDeleteIngresoGasto: React.FC<ModalDeleteIngresoGastoProps> = ({
  isOpen,
  onClose,
  onConfirm,
  type,
    ...
    <div className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-50 z-50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md w-full transform transition-all duration-300 scale-95 hover:scale-100">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2">¿Eliminar {capitalizedType}?</h2>
          <p className="text-gray-600 mb-6">
            Esta acción eliminará permanentemente el {type} y no podrá recuperarse.
          </p>
        </div>
...
export default ModalDeleteIngresoGasto;
