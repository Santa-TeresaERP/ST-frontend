...
export const warehouseMovementResourceSchema = z.object({
  id: z.string().uuid('El ID del movimiento debe ser un UUID válido').nonempty('El ID del movimiento no puede estar vacío'), // Renamed from movement_id
  ...
  movement_type: z.string().min(1, 'El tipo de movimiento es obligatorio').max(50, 'El tipo de movimiento no debe exceder los 50 caracteres'), // 'type' field removed
