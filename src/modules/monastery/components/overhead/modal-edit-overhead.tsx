...
const ModalEditOverhead: React.FC<Props> = ({ isOpen, onClose, overheadToEdit }) => {
  ...
  useEffect(() => {
    if (overheadToEdit) {
      ...
      reset({ 
        ...
        amount: overheadToEdit.amount,
        date: formattedDate,
        description: overheadToEdit.description || ''
      });
    }
  ...
  const onSubmit: SubmitHandler<UpdateOverheadFormData> = (data) => {
    ...
    updateOverhead({ id: overheadToEdit.id, payload: data }, {
      ...
      onError: (error) => {
        console.error("Error al actualizar el gasto general:", error);
      },
    });
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md">
        <div className="flex justify-between items-center p-4 border-b">
          <h3 className="text-xl font-semibold text-gray-800">Editar Gasto General</h3>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="flex justify-end items-center p-4 border-t">
            ...
            <button type="submit" disabled={isPending} className="px-4 py-2 bg-red-700 text-white rounded-md hover:bg-red-800 disabled:opacity-50">
              {isPending ? 'Actualizando...' : 'Actualizar Gasto General'}
            </button>
          </div>
        </form>
      </div>
...
export default ModalEditOverhead;