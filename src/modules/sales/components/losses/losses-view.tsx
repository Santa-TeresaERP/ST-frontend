...
} from "@/modules/sales/utils/store-status";
...
const LossesComponentView: React.FC<LossesComponentViewProps> = ({
  ...
  const { data: storeSessionData, isLoading: isLoadingSessionData } =
    useCheckStoreActiveSession(selectedStoreId);

  // Permisos del módulo
  const { canCreate, canEdit, canDelete, isAdmin } = useModulePermissions(
    MODULE_NAMES.SALES
  );
  ...
  // ✅ Filtro: mostrar pérdidas de la tienda seleccionada, o todas si no hay selección
...
console.log("DEBUG losses:", losses.map(l => ({ id: l.id, storeId: l.storeId })));
    ...
    <div className="bg-white p-6 rounded-xl shadow-md border border-gray-200 space-y-6 text-gray-700">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
        <small className="text-blue-700">
          <strong>Estado Permisos:</strong> Módulo: {MODULE_NAMES.SALES} |
          Crear: {canCreate ? "✅ SÍ" : "❌ NO"} | Editar:{" "}
          {canEdit ? "✅ SÍ" : "❌ NO"} | Eliminar:{" "}
          {canDelete ? "✅ SÍ" : "❌ NO"} | Admin: {isAdmin ? "✅ SÍ" : "❌ NO"}
        </small>
      </div>
      ...
      {!selectedStoreId && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
          <p className="text-yellow-800">
            ...
            panel principal para gestionar las pérdidas.
          </p>
        </div>
      )}
      ...
      {!canCreate && (
        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
          <p className="text-red-800">
            🚫 <strong>Sin permisos:</strong> No tienes permisos para crear
            nuevas pérdidas.
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-purple-50 p-4 rounded-lg border border-purple-100">
          ...
          <p className="text-2xl font-bold text-purple-600">
            {
              filteredLosses.filter((item) => item.reason === "Vencimiento")
                .length
            }
          </p>
        </div>
        <div className="bg-pink-50 p-4 rounded-lg border border-pink-100">
          ...
          <p className="text-2xl font-bold text-pink-600">
            {
              filteredLosses.filter((item) => item.reason === "Transporte")
                .length
            }
          </p>
        </div>
